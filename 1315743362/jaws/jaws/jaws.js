function Jaws(jsdo) {
	...
	var ie_sub8 = navigator.appVersion.match(/MSIE (6|7)/);
	var thiss = this;
	var problem;
	...
	$.each(this.data, function(key, val) {
		...
		(li = $(document.createElement('li')))
			.css(!ie_sub8 ? null : {padding: 7})
			.appendTo(thiss.ul);
			
		if (val.title) //else is padder (i.e. empty) LI
			$(document.createElement('a'))
				.html((jsdo.doNumbers ? (key+1)+' - ' : '')+val.title)
				.attr({href: val.moreInfo ? 'javascript:void(0)' : val.url, target: !val.moreInfo && val.new_window == 1 ? '_blank' : ''})
				.appendTo(li)
				...
				.click(function() {
					...
					li.slideDown(Jaws.config.animDuration, function() {
						...
						$(document.createElement('a'))
							.appendTo(infoBottom)
							.attr({href: val.url, target: val.new_window == 1 ? '_blank' : ''})
							.css('float', 'right')
	...
	var tallestLISoFar = false;
	this.ul.children().each(function() { if (!tallestLISoFar || $(this).height() > tallestLISoFar.height()) tallestLISoFar = $(this); });
	this.itemHeight_offset = tallestLISoFar.get(0).offsetHeight;
	this.ul.children().height(this.itemHeight = tallestLISoFar.height());
	this.ul.css('height', tallestLISoFar.get(0).offsetHeight * this.itemsPerFold);
	...
	for (var j=0; j<numPages+2; j++)
		$(document.createElement('li'))
			...
			.click(function() {
				...
				var newScrollTop = ($(this).index() - 1) * thiss.itemsPerFold * thiss.itemHeight_offset;
